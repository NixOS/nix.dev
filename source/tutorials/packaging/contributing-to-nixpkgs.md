...
3. Use `nix-prefetch-url` (comes with any Nix installation) or [another `nix-prefetch-*` tool](https://search.nixos.org/packages?type=packages&query=nix-prefetch) to fetch, unpack, and hash the files marked by the tag or release you selected.
4. Alternatively, you can also use a tool such as [`nix-init`](https://github.com/nix-community/nix-init) to automatically generate a required derivation for some common project types, or use its output to help lay down a skeleton implementation of the above steps.
